...
class MessageHandler {
    ...
    async sendMessageToUser(sessionData, messageContent) {
        ...
        try {
            ...
            if (messageContent.length > MAX_MESSAGE_LEN) {
                // this is a hack - it assumes the message breaks in the middle of a triple backtick
                message = await user.send(messageContent.slice(0,MAX_MESSAGE_LEN - 4) + '```')
                
                // part 2 with warning
                await user.send('```' + messageContent.slice(MAX_MESSAGE_LEN - 4) + `\n:warning: This last message was too long, so I had to break it up into 2 parts.  It might look a little weird.`)

            } else {
                message = await user.send(messageContent)
            }
            ...
            try {
                ...
                let message = await user.send(`:warning: Sorry. I ran into a ${e.httpStatus || 'unknown'} error while trying to send this message. I don't know what happened.`)
            } catch (e) {
                logger.error('Failed to DM: ' + e + "\n" + e.stack)
