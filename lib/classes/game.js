...
class Game {
    ...
    static fromJson(jsonData) {
        ...
        if (jsonData.teams != null) {
            for (let teamColor in jsonData.teams) {
                ...
                let team = new Team(teamData.color, teamData.name, teamData.leagueName, teamData.teamId)

                // add players
                if (teamData.players != null) {
                    for (let playerData of teamData.players) {
                        let player = new Player(playerData.gamertag, playerData.discordName, playerData.goals, playerData.score, playerData.assists, playerData.saves, playerData.shots, playerData.mvp, playerData.sub, playerData.gid)
    ...
    setPlayerGidByGamertag(gid, gamertag) {
        this.callbackTeamAndPlayerByGamertag(gamertag, (u) => {
            u.player.gid = gid
            return u
        })
    ...
    setTeamLeagueNameAndTeamIdByGamertag(leagueName, teamId, gamertag) {
        const matched = this.callbackTeamAndPlayerByGamertag(gamertag, (u) => {
            ...
            u.team.teamId = teamId
            return u
        })
    }

    setTeamLeagueNameAndTeamIdByDiscordName(leagueName, teamId, discordName) {
    ...
    updateWinningTeamAndMVP() {
        ...
        for (let team of teamsInOrder) {
            ...
            if (team.winner) {
                let highestScore = 0
                for (let player of team.players) {
                    const playerScoreInt = parseInt(player.score)
                    if (playerScoreInt > highestScore) {
                        mvpPlayer = player
                        // console.log('mvpPlayer set to ',player.gamertag,' with score '+`${player.score} > ${highestScore}`)
                        highestScore = playerScoreInt
                    }
                }
            }
