...
module.exports.init = function(messageHandler, config) {
    ...
    Command.help = 'Send a stat line for the game.  You can replace a stat line by entering a new stat with the same player name again.  Use the in-game player name to match players.  To prevent automatic player matching, enter the player name starting with a \`*\`.'
    ...
    Command.handle = async function(sessionData, msg) {
        ...
        try {
            ...
            if (gamertag.substring(0,1) == '*') {
                // force match
                matchedGamertag = gamertag.substring(1)
                discordName = gamertag.substring(1)
            } else {
                ...
                matchedPlayer = matchedPlayerResults[0].player
                // console.log('matchedPlayer', JSON.stringify(matchedPlayer,null,2))
            }

            if (matchedPlayer != null) {
                matchedGamertag = matchedPlayer.gamertag
            }
        ...
        const newPlayerGamertag = matchedGamertag === null ? gamertag : matchedGamertag
        const newPlayerDscordName = matchedPlayer === null ? discordName : matchedPlayer.discordName
        const newPlayer = new Player(newPlayerGamertag, newPlayerDscordName, goals, score, assists, saves, shots)
