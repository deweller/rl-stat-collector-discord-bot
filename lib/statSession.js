...
StatSession.init = function(client, config) {
    ...
    statSession.handleDirectMessage = async function(msg) {
        let sessionData = await initStatSessionByMsg(msg)
        for (let commandMsg of splitMessageIntoMultipleCommands(msg)) {
            await statSession.handleCmdMessage(sessionData, commandMsg)
        }
    }

    statSession.handleCmdMessage = async function(sessionData, msg) {
        try {
    ...
    function splitMessageIntoMultipleCommands(msg) {
        const commandMessages = []

        const lines = msg.content.trim().split("\n");
        if (lines.length > 1 && lines[0].trim().toLowerCase() == 'multi') {
            for (let line of lines.slice(1)) {
                // clone the message
                const newMsg = Object.assign({}, msg, {content: line});
                commandMessages.push(newMsg)
            }
        } else {
           // just one line
        ...
        // console.debug('commandMessages', JSON.stringify(commandMessages))
