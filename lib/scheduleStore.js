...
exports.currentWeek = function(now = null) {
    ...
    for (let week of reversedWeeks) {
        // console.log(`week: ${week} startDate: ${exports.getStartDateByWeek(week, 6 - BUFFER_DAYS)}`)
...
exports.getStartDateByWeek = function(week, daysBefore = 6) {
    ...
    // console.log(`row.date: ${row.date} parsed: ${parse(row.date, 'E MMM d yyyy h:mm a', new Date())}`)
    const dateObj = zonedTimeToUtc(parse(row.date, 'E MMM d yyyy h:mm a', new Date()), serverTz)
